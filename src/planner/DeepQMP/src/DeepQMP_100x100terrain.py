...
def TerrainCallback(msg):
    ...
    TERRAIN[0,0]=torch.linspace(-5,5,re_size).view(1,-1).repeat(re_size,1)
    TERRAIN[0,1]=torch.linspace(-5,5,re_size).view(-1,1).repeat(1,re_size)
...
'''
class DeepQMP(nn.Module): # torch.nn.Module을 상속받는 파이썬 클래스
    input_dim=40   # Initial robot state, 40
    goal_dim=6     # Goal base body pose, 6
    output_dim=40 # Each node's dimension of output trajectory, 40

    input_dim=input_dim + goal_dim  # Input dimension for state feature extraction, 46
    ...
    def __init__(self):
        ...
        dslayer3 = 256
        self.CNN_DenseNet = nn.Sequential(
                                                nn.Linear(DeepQMP.input_dim, dslayer1, bias=True),
                                                nn.BatchNorm1d(dslayer1),
                                                mish(),
        ...
        self.PathLen = (MaxSampleNum-1)//dt
        self.GenPath = nn.Sequential(
                                                nn.Linear(dslayer3, self.PathLen*DeepQMP.output_dim),
                                      )
    ...
    def forward(self, X, Terrain):
        ...
        # MLP
        mlpX = X.view(-1,DeepQMP.input_dim)
        MLPout = self.CNN_DenseNet(mlpX)
...
def EstimatePath(Encoder, Planner, Init, terr, pub):
    ...
    Env = Encoder(terrain.view(-1,1,re_size,re_size))[-1]
    # ts=int(round(time.time() * 1000))
    ...
    for feet in FEET:
        ...
        feetX=Motion[0,:,12+feet].view(-1,1);   idx=((feetX-Xmin)//res).int().tolist()
        feetY=Motion[0,:,13+feet].view(-1,1);   idy=((feetY-Ymin)//res).int().tolist()
...
if __name__=='__main__':
    ...
    params = torch.load("/home/mrjohd/Quadrupeds_ws/src/planner/DeepQMP/model/QuadNet.tar", map_location = "cpu")
    # params = torch.load("/home/mrjohd/Quadrupeds_ws/src/planner/DeepQMP/model/QuadNet_NoFeature.tar", map_location = "cpu")
