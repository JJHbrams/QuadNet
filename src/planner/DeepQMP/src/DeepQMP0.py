...
def GetheightT(inX, inY, Terr):
    thres = 0.1
    bc=inX.shape[0]
    desX=inX
    desY=inY
    Terrain=Terr.repeat(bc,1,1,1)
    ycor=torch.where(abs(Terrain[:,0]-desX.view(-1,1,1,1).repeat(1,1,64,64))<thres)
    ycor = ycor[-1].max()
    xcor=torch.where(abs(Terrain[:,1,:,ycor].view(-1,1,64,1)-desY.view(-1,1,1,1).repeat(1,1,64,1))<thres)
    xcor=xcor[2][0].item()
    return Terrain[:,2,xcor,ycor]
...
def interNodes(Nodes, batch_size, seq_size=MaxSampleNum//dt):
  ...
  InterState=F.interpolate(data[:,:-4,:,:], [1,MaxSampleNum], mode='bicubic') # 4D 1-dim height form
...
def EstimatePath(Encoder, Planner, Init, terr, pub):
    ...
    # Goal[0,2] += Getheight(Goal[:,0],Goal[:,1],terr[0])
    ...
    # data=Nodes.view(1,1,seq_size,Path_dim).permute(1,3,0,2)
    # PredictS=F.interpolate(data[:,:-4,:,:], [1,MaxSampleNum], mode='bicubic') # Continuous States
    # PredictC=F.interpolate(data[:,-4:,:,:], [1,MaxSampleNum], mode='nearest') # Discrete Contacts
    # Predict = torch.cat([PredictS, PredictC], dim=1)
    # Predict=Predict.permute(2,0,3,1).view(1,MaxSampleNum,Path_dim)
    ...
    for feet in FEET:
        ...
        Hterr=GetheightT(Motion[0,:,12+feet].view(-1,1),Motion[0,:,13+feet].view(-1,1),terr[0])
        sign=torch.tensor(Motion[0,:,14+feet] >= Hterr, dtype=torch.float)
        Motion[0,:,14+feet] = sign*Motion[0,:,14+feet] + (1-sign)*(Hterr+Motion[0,:,14+feet])
        # Revise feet force z
...
if __name__=='__main__':
    ...
    tempX=torch.linspace(-5,5,64).view(1,-1).repeat(64,1)
    tempY=torch.linspace(-5,5,64).view(-1,1).repeat(1,64)
    img[0,0]=tempX
    img[0,1]=tempY
    ...
    while not rospy.is_shutdown():
        spinOnce(10000)
        if path_flag[-1]==True:
            EstimatePath(Encoder, Planner, Init, img, pub_traj)
